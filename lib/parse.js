...
const constants = {
  ...
  NEWLINE_REGEX: /^\r*\n/,
  LINE_FREE_OPEN_REGEX: /^<style(\s+[^>]*)?>/,
  LINE_FREE_CLOSE_REGEX: /^<\/style(\s+[^>]*)?>/,
};
...
const parse = (input, options = {}) => {
  ...
  while (remaining !== '') {
    ...
    // line comment free block open
    if (!tripleQuotes && block.type !== 'block') {
      if ((token = scan(constants.LINE_FREE_OPEN_REGEX, 'open'))) {
        push(new Block({ type: 'linefree' }));
        push(new Node(token));
        continue;
      }
    }

    //line comment free block close
    if(block.type === 'linefree') {
      if ((token = scan(constants.LINE_FREE_CLOSE_REGEX, 'close'))) {
        push(new Node(token));
        pop();
        continue;
      }
    ...
    // line comment
    if (LINE_REGEX && block.type !== 'block' && block.type !== 'linefree' && options.line) {
